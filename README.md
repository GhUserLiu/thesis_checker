...
论文选题查重系统是一个专门用于检测毕业设计/论文选题相似度的Python工具。它能够自动读取Excel文件中的选题信息，使用自然语言处理技术计算选题之间的相似度，并生成详细的查重报告。

### ✨ 核心特性

- 🔍 **智能相似度检测** - 基于TF-IDF和余弦相似度算法
- 📊 **多格式支持** - 支持 `.xlsx`, `.xls` 格式
- 🎯 **自动班级识别** - 从文件名自动提取班级信息
- 🧹 **数据清洗** - 自动过滤无效题目（待定、入伍等）
- 📈 **详细报告** - 生成包含多个工作表的Excel报告
- 🎨 **颜色标注** - 相似题目黄色高亮，无效题目灰色标记
- 📝 **数据规范化** - 统一专业、组织等字段格式
- 🔄 **二次提交支持** - 支持修改后数据的合并查重
- 🧩 **智能合并** - 按学生姓名+学号精确识别，自动合并重复记录
- 📊 **排序输出** - 按班级和学号自动排序
...
2. 同时读取原始数据文件夹作为参考
3. **学生合并规则**：按"学生姓名 + 学号"判断是否为同一学生
   - 以二次提交的**课题名称**为准
   - 如果二次提交中某些字段缺失，从原始数据中补充
   - 最终每个学生只保留一条记录（同名但不同学号视为不同学生）
...
1. **唯一标识**：使用"学生姓名 + 学号"作为唯一标识
2. **合并优先级**：二次提交 > 原始数据
3. **字段补充**：二次提交中缺失的字段从原始数据补充（课题名称除外）
4. **去重**：同名但不同学号的学生被视为不同的学生
...
| 字段 | 规范化规则 |
|------|----------|
| 课题名称 | 去除所有空格、制表符、NBSP、全角空格、零宽字符等 |
...
根据文件名中的班级信息或原始专业名称自动判断：
- 包含"新能"或"新能源" → 新能源汽车工程技术
- 包含"汽服"或"汽车服务" → 汽车服务工程技术
- 其他 → 汽车服务工程技术（默认）
...
**说明**：系统按"姓名+学号"判断是否为同一学生。如果学号不同，即使是同名也会被视为不同的学生。这是设计行为，用于处理同名不同人的情况。
...
- 确保二次提交文件夹中有Excel文件
- 二次提交文件夹不为空时才会启用
- 检查控制台输出确认是否使用了二次提交数据

## 📦 打包为可执行文件

如需将项目打包为独立的可执行文件（.exe），可以使用 PyInstaller：

```bash
pip install pyinstaller
pyinstaller --onefile checker.py
```
